...
def get_model_actor_pretrained(input_dims, output_dims):
...
def get_model_critic_pretrained(input_dims):
    state_input = Input(shape=input_dims)
...
model_actor = get_model_actor_pretrained(input_dims=state_dims, output_dims=n_actions)
model_critic = get_model_critic_pretrained(input_dims=state_dims)
...
while not target_reached and iters < max_iters:
    ...
    actor_loss = model_actor.fit(
        [states, action_probs, advantages, np.reshape(rewards, newshape=(-1, 1, 1)), values[:-1]],
        [one_hot_encoding(np.reshape(action_probs, newshape=(-1, n_actions)))], verbose=True, shuffle=True, epochs=8,
